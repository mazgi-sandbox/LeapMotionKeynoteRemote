...
@interface Sample ()
@property (nonatomic, retain) KeynoteRunner *runner;
...
- (void)run
{
    self.runner = [[KeynoteRunner alloc]init];
...
{
    NSLog(@"Initialized");
    [(LeapController *)[notification object]setPolicyFlags:LEAP_POLICY_BACKGROUND_FRAMES];
}
...
//                      swipeGesture.position, swipeGesture.direction, swipeGesture.speed);
                NSLog(@"[%05d]startPosition: %@, position: %@ [%@]",
                      swipeGesture.id,
                      swipeGesture.startPosition,
                      swipeGesture.position,
                      [Sample stringForState:swipeGesture.state]);
//                NSLog(@"x: %f, %f -> %f",
//                      swipeGesture.startPosition.x - swipeGesture.position.x, swipeGesture.position.x, swipeGesture.direction.x);
//                NSLog(@"y: %f, %f -> %f",
//                      swipeGesture.startPosition.y - swipeGesture.position.y, swipeGesture.position.y, swipeGesture.direction.y);
//                NSLog(@"z: %f, %f -> %f",
//                      swipeGesture.startPosition.z - swipeGesture.position.z, swipeGesture.position.z, swipeGesture.direction.z);
                if (swipeGesture.state == LEAP_GESTURE_STATE_STOP) {
                    if (fabs([self.lastTime timeIntervalSinceNow]) >= 0.5) {
                        float dx = swipeGesture.startPosition.x - swipeGesture.position.x;
                        float dy = swipeGesture.startPosition.y - swipeGesture.position.y;
                        if (fabs(dx) > fabs(dy) * 1.4) {
                            //horizontal
                            if (dx > 10.0) {
                                [self.runner showNext:nil];
                            } else if(dx < 10.0) {
                                [self.runner showPrevious:nil];
                            }
                        } else if (fabs(dx) * 1.4 < fabs(dy)) {
                            //vertical
                            if (dy > 10.0) {
                                [self.runner stopSlideshow:nil];
                            } else if(dy < 10.0) {
                                [self.runner startFrom:nil];
                            }
                        }
                        self.lastTime = [NSDate date];
                    }
